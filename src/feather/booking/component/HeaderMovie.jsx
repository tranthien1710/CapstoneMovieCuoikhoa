...
const HeaderMovie = () => {
    ...
    return (
        <header className='flex justify-between items-center bg-[#141414] text-white'>
            ...
            {profile ? <span className='text-white mr-10' > Xin chào : {profile.hoTen}</span> : <div className='mr-5 cursor-pointer '>
                <NavLink to="/login" className={({ isActive }) => isActive ? 'text-3xl no-underline text-white' : 'text-white no-underline'} > <span className='hover:text-[#b3b3b3]'>Đăng kí</span></NavLink>
                <NavLink to='/sign' className={({ isActive }) => isActive ? 'text-3xl no-underline text-white' : 'text-white no-underline'} > <span className='hover:text-[#b3b3b3]'> | Đăng nhập</span></NavLink>
            </div>}
